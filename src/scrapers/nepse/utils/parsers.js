...
function formatCSVDownloadData(data) {
  ...
  return data.map(record => {
    ...
    return {
      ...
      securityId: record.securityId || record.security_id,
...
function formatAPIData(stockArray) {
  return stockArray.map(stock => {
    ...
    return {
      ...
      securityId: stock.securityId || stock.security_id,
      ...
      change: pointChange,
      percentageChange: percentChange,
      totalTradedQuantity: stock.totalTradedQuantity || 0,
      totalTradedValue: stock.totalTradedValue || 0,
      totalTrades: stock.totalTrades || 0,
      averageTradedPrice: stock.averageTradedPrice || 0,
      marketCapitalization: stock.marketCapitalization || 0,
      fiftyTwoWeekHigh: stock.fiftyTwoWeekHigh || 0,
      fiftyTwoWeekLow: stock.fiftyTwoWeekLow || 0,
      lastUpdatedTime: stock.lastUpdatedTime,
      lastTradedPrice: ltp,
      volume: stock.totalTradedQuantity || 0,
      turnover: stock.totalTradedValue || 0,
      maxPrice: stock.highPrice || 0,
      minPrice: stock.lowPrice || 0
    };
  });
}
...
function formatHTMLData(rawData) {
  ...
  return rawData.map(row => {
    ...
    return {
      ...
      securityId: parseNumber(row.securityid || row.security_id || 0),
