...
class NepseScraper {
  ...
  async scrapeMarketSummary() {
    console.log('🔍 Checking market summary (status + index)...');
    ...
    try {
      ...
      // --- 1. Scrape Status ---
      console.log('📖 Reading market status...');
      const bodyText = await page.evaluate(() => document.body.innerText);
      ...
      // Status logic
      const isPreOpen = bodyText.includes('Pre Open') || /Market Status[:\s]*PRE[- ]?OPEN/i.test(bodyText);
      const isMktOpen = bodyText.includes('Market Open') || /Market Status[:\s]*OPEN/i.test(bodyText);
      const isClosed = bodyText.includes('Market Closed') || /Market Status[:\s]*CLOSED/i.test(bodyText) || bodyText.includes('3:00:00 PM');
      ...
      if (isPreOpen) {
        ...
        if (!isTradingDay) {
          status = 'CLOSED';
        } else if (currentTime >= 1030 && currentTime < 1045) {
          status = 'PRE_OPEN';
          isOpen = true;
        } else if (currentTime >= 1045 && currentTime <= 1500) {
          status = 'OPEN';
          isOpen = true;
        }
      }
      ...
      try {
        await page.waitForSelector('.index-value, .market-index, .current-index', { timeout: 5000 });
      } catch (e) {
        console.log('⚠️ Index selector not immediately found, trying fallbacks...');
      }
      ...
      const indexData = await page.evaluate(() => {
        ...
        const indexEl = document.querySelector('.current-index-value') ||
          document.querySelector('.index-value') ||
          Array.from(document.querySelectorAll('div, span, td')).find(el => el.textContent.includes('NEPSE Index') && el.textContent.length < 50);

        if (indexEl) {
          const parent = indexEl.closest('.index-row') || indexEl.parentElement;
          const valueText = parent ? parent.innerText : '';
          const matches = valueText.match(/([\d,]+\.?\d*)/g);
          if (matches && matches.length > 0) {
            nepseIndex = parseNum(matches[0]);
          }
        }

        if (nepseIndex === 0) {
          const matches = document.body.innerText.match(/NEPSE Index\s*[\n\r\s]+([\d,]+\.?\d*)/i);
          if (matches) nepseIndex = parseNum(matches[1]);
        }

        let turnover = 0;
        const turnoverMatch = document.body.innerText.match(/Total Turnover\s*[\n\r\s]+(Rs\.)?\s*([\d,]+\.?\d*)/i);
        if (turnoverMatch) turnover = parseNum(turnoverMatch[2]);
      ...
      // Try for detailing
      let detailedIndexData = {};
      try {
        detailedIndexData = await page.evaluate(() => {
          const result = {};
          const summaryText = document.body.innerText;

          // Adv/Dec/Unc
          const advMatch = summaryText.match(/Advanced[:\s]+(\d+)/i);
          if (advMatch) result.advanced = parseInt(advMatch[1], 10);

          const decMatch = summaryText.match(/Declined[:\s]+(\d+)/i);
          if (decMatch) result.declined = parseInt(decMatch[1], 10);

          const uncMatch = summaryText.match(/Unchanged[:\s]+(\d+)/i);
          if (uncMatch) result.unchanged = parseInt(uncMatch[1], 10);
      ...
      const finalIndexData = {
        ...indexData,
        ...detailedIndexData,
        nepseIndex: detailedIndexData.nepseIndex || indexData.nepseIndex || 0,
        totalTurnover: detailedIndexData.totalTurnover || indexData.totalTurnover || 0
      };

      const now = DateTime.now().setZone('Asia/Kathmandu');
      finalIndexData.tradingDate = now.toISODate();
      finalIndexData.marketStatusTime = now.toFormat('HH:mm:ss');
      finalIndexData.marketStatusDate = now.toISODate();
      ...
      console.error('❌ Market summary check failed:', error.message);
      throw error;
    } finally {
      if (page) {
        await page.close().catch(() => { });
  ...
  // Legacy support
  async scrapeMarketStatus() {
    const summary = await this.scrapeMarketSummary();
    return summary.status;
  }

  async scrapeMarketIndex() {
    const summary = await this.scrapeMarketSummary();
    return summary.indexData;
  }
}
